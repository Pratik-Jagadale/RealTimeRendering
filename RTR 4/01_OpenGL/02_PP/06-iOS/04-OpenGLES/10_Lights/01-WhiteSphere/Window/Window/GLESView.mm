...
- (int)initialize
{
	...
	glBindAttribLocation(shaderProgramObject, PRJ_ATRIBUTE_NORMAL, "a_noraml");
...
- (void)display
{
	...
	scalenMatrix = vmath::scale(0.1f, 0.1f, 0.1f); // glTranslatef() is replaced by this line

	// modelViewMatrix = translationMatrix * scalenMatrix; // order is very important
	modelViewMatrix = translationMatrix ; // order is very important

	modelViewProjectionMatrix = perspectiveProjectionMatrix * viewMatrix *  modelViewMatrix;

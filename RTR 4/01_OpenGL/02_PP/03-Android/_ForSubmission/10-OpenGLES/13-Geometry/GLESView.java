...
public class GLESView extends GLSurfaceView implements OnDoubleTapListener, OnGestureListener, GLSurfaceView.Renderer {
    ...
    private void initialize() {
        ...
        // Geometry Shader
        final String geometryShaderSourceCode = new String(
                "#version 320 es" +
                        "\n" +
                        "precision highp float;" +
                        "layout(triangles)in;" +
        ...
        if (status[0] == GLES32.GL_FALSE) {
            ...
            if (infoLogLength[0] > 0) {
                log = GLES32.glGetProgramInfoLog(geometryShaderObject);
                System.out.println("PRJ: GL_GEOMETRY_SHADER COMPILATION LOG :" + log);
                uninitialize();
                System.exit(0);
            }
        }

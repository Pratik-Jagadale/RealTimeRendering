...
public class GLESView extends GLSurfaceView implements OnDoubleTapListener, OnGestureListener, GLSurfaceView.Renderer {
    ...
    private int vbo_Indices[] = new int[1];
    ...
    private void initialize() {
        ...
        GLES32.glGenBuffers(1, vbo_Indices, 0);
        GLES32.glBindBuffer(GLES32.GL_ELEMENT_ARRAY_BUFFER, vbo_Indices[0]);

        byteBuffer = ByteBuffer.allocateDirect(squareIndices.length * 4); // 4 = float size
        byteBuffer.order(ByteOrder.nativeOrder());
        IntBuffer indicesbuffer = byteBuffer.asIntBuffer();
        indicesbuffer.put(squareIndices);
        indicesbuffer.position(0);

        GLES32.glBufferData(GLES32.GL_ELEMENT_ARRAY_BUFFER, squareIndices.length * 4, indicesbuffer,

...
void display(void)
{
	...
	/*
		for (float i = -4.0f; i < 4.0f; i = i + 1.3f)
		{

			for (float j = 0.0f; j < 5.0f; j = j + 1.2f)
			{

				translationMatrix = vmath::translate(i, 0.0f - 1.5f + j, -6.0f); // glTranslatef() is replaced by this line
...
void update(void)
{
	...
	lightDirection[1] = 2.5f;
	lightDirection[2] = -6.0f;
}

...
int initialize(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER, new_mash_data->gp_normal->size * sizeof(float), new_mash_data->gp_normal->p_arr, GL_STATIC_DRAW);
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, new_mash_data->gp_normal_indices->size * sizeof(int), new_mash_data->gp_vertex_indices->p_arr, GL_STATIC_DRAW);
...
void display(void)
{
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, gVbo_sphere_element);
	glDrawElements(GL_TRIANGLES, new_mash_data->gp_vertex_filter->size, GL_UNSIGNED_INT, 0);

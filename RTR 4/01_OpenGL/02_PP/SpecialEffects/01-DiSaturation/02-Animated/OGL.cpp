...
GLuint conversionRateUniform;

GLuint resolutionWidthUniform;
GLuint resolutionHeightUniform;
GLuint greyscaleAnimateFactorUniform;

BOOL bConvertToGrayScale;
BOOL bAnimateGreyscale = FALSE;
BOOL bReset = FALSE;

unsigned int giResolutionWidth;
unsigned int giResolutionHeight;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
		...
		case 'A':
		case 'a':
			if (bAnimateGreyscale == TRUE)
			{
				bAnimateGreyscale = FALSE;
			}
			else
			{
				bAnimateGreyscale = TRUE;
			}
			break;

		case 'g':
		case 'G':
			if (bConvertToGrayScale == TRUE)
			{
				bConvertToGrayScale = FALSE;
			}
			else
			{
				bConvertToGrayScale = TRUE;
			}
			break;
...
int initialize(void)
{
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"uniform float u_conversionRate;"
		"uniform float u_resolutionWidth;"
		"uniform float u_resolutionHeight;"
		"uniform float u_greyscaleAnimateFactor;"
		"out vec4 FragColor;"
		"void main(void)"
		"{"
		"vec4 color;"
		"color = texture(u_textureSampler , a_texcoord_out);"
		"if((gl_FragCoord.y / u_resolutionHeight) <  u_greyscaleAnimateFactor)"
		"{"
		"float grayscale = (color.r * 0.3) + (color.g * 0.59) + (color.b * 0.11);"
		"vec4 grayscale_color = vec4(grayscale,grayscale,grayscale, 1.0);"
		"FragColor = mix(color,grayscale_color, u_conversionRate);"
		"}"
		"else"
		"{"
		"FragColor = color;"
	...
	conversionRateUniform = glGetUniformLocation(shaderProgramObject, "u_conversionRate");
	resolutionWidthUniform = glGetUniformLocation(shaderProgramObject, "u_resolutionWidth");
	resolutionHeightUniform = glGetUniformLocation(shaderProgramObject, "u_resolutionHeight");
	greyscaleAnimateFactorUniform = glGetUniformLocation(shaderProgramObject, "u_greyscaleAnimateFactor");
...
void resize(int width, int height)
{
	...
	giResolutionHeight = height;
	giResolutionHeight = width;
...
void display(void)
{
	...
	static GLfloat converisionRate = 0.0f;
	static GLfloat greyscaleAnimateFactor = 1.0f;
	...
	glUniform1f(conversionRateUniform, converisionRate);

	glUniform1f(resolutionWidthUniform, (GLfloat)giResolutionWidth);
	glUniform1f(resolutionHeightUniform, (GLfloat)giResolutionHeight);

	if (bConvertToGrayScale)
	{
		if (converisionRate <= 1.0f)
			converisionRate = converisionRate + 0.005f;

		if (bAnimateGreyscale == TRUE)
		{
			glUniform1f(greyscaleAnimateFactorUniform, greyscaleAnimateFactor);

			if (greyscaleAnimateFactor > 0.0f)
			{
				greyscaleAnimateFactor = greyscaleAnimateFactor - 0.005f;
			}
		}
		else
		{
			...
			if (greyscaleAnimateFactor > 1.0f)
			{
				greyscaleAnimateFactor = greyscaleAnimateFactor + 0.005f;
			}
		}

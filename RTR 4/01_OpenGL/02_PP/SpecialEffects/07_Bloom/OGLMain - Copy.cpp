...
int initialize(void)
{
...

	{
		...
		BOOL bShaderLinkStatus = LinkShaderProgramObject(shaderProgramObject_PV);
...
void display(void)
{
	...
	// translationMatrix = vmath::translate(j - 2.0f, -1.0f, -4.0f - ); // glTranslatef() is replaced by this line
	...
	/*
		if (bPerFragShader == TRUE)
		{
			translationMatrix = mat4::identity();
			modelMatrix = mat4::identity();
			viewMatrix = mat4::identity();

			translationMatrix = vmath::translate(lightPositions[0], lightPositions[1], lightPositions[2]); // glTranslatef() is replaced by this line

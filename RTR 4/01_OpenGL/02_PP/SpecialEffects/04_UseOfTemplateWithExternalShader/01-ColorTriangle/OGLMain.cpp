...
int initialize(void)
{
	...
	// vertex Shader
	GLuint vertexShaderObject = CreateAndCompileShaderObjects("Shaders\\ColorTriangle.vs", VERTEX);

	// fragment Shader
	GLuint fragmentShaderObject = CreateAndCompileShaderObjects("Shaders\\ColorTriangle.fs", FRAGMENT);
	...
	// Binding Color Array
	glBindAttribLocation(shaderProgramObject, PVG_ATTRIBUTE_COLOR, "a_color");
	...
	glVertexAttribPointer(PVG_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(PVG_ATTRIBUTE_COLOR);

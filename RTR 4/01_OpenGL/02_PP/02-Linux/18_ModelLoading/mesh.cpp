#include "mesh.h"
#include <stdlib.h>
#include <stdio.h> // For fopen_s(), f//fprintf(gpFile,), fclose()
#include <string.h>
...
mesh_data_t *load_mesh(const char *path)
{
    ...
    {
        fprintf(gpFile, "load_mesh() FAILED... ERROR IN OPENING FILE...\n");
        return NULL;
    }
    else
    {
        fprintf(gpFile, "load_mesh() ...OPENING FILE...\n");
    }
    ...
    if (p_mesh_data == NULL)
    {
        fprintf(gpFile, "p_mesh_data == null...\n");
        return NULL;
    }
...
void free_mesh_data(mesh_data_t **pp_mesh_data)
{
    ...
    fprintf(gpFile, "OBJ Data Destroyed Successfulyy..");
}
...
mesh_data_t *create_mesh_data(void)
{
    ...
    if (p_mesh_data == NULL)
    {
        fprintf(gpFile, "create_mesh_data FAILED... malloc():OUT OF MEMORY.\n");
        return (NULL);
    }
...
vec_int_t *create_vec_int(void)
{
    ...
    if (p == NULL)
    {
        fprintf(gpFile, "create_vec_int FAILED... malloc():OUT OF MEMORY.\n");
        return (NULL);
    }
...
vec_float_t *create_vec_float(void)
{
    ...
    if (p == NULL)
    {
        fprintf(gpFile, "create_vec_float FAILED... - malloc() OUT OF MEMORY.\n");
        return (NULL);
    }
...
int main(void)
{
    ...
    if (new_mash_data == NULL)
        //fprintf(gpFile,"Error \n");
    else
        //fprintf(gpFile,"DATA READED SUCCEESFULLY\n");

    int i = 0;

    //fprintf(gpFile,"gp_vertex->size : %d\n", new_mash_data->gp_vertex->size);
    //fprintf(gpFile,"gp_vertex->size : %d\n", new_mash_data->gp_vertex->size);
    //fprintf(gpFile,"gp_vertex->size : %d\n", new_mash_data->gp_vertex->size);
    ...
    while (i < new_mash_data->gp_vertex->size)
    {
        ...
        for (j = 0; j < 3; j++)
        {
            //fprintf(gpFile,"%f,", new_mash_data->gp_vertex->p_arr[i]);
        }
        //fprintf(gpFile,"\n");
        i = i + 3;
    }

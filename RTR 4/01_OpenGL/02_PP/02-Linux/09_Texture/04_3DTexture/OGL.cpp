...
GLuint vbo_Pyramid_Texcoord; // Vertex Buffer Object - Pyramid
...
int main(void)
{
	...
	{
		fprintf(gpFile, "Error - loadGLTexture() Failed for stone..\n");
		uninitiallize();
		exit(0);
	}
	else if (ret == -3)
	{
		fprintf(gpFile, "Error - loadGLTexture() Failed kundali..\n");
		uninitiallize();
		exit(0);
	}

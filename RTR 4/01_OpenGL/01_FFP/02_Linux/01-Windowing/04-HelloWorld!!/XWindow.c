...
int main(void)
{
	...
	windowAttribute.colormap = XCreateColormap(	display,
												...
												visualInfo.visual,
												AllocNone);
	windowAttribute.event_mask = ExposureMask | KeyPressMask | StructureNotifyMask;
	...
	while(1)
	{
		...
		case Expose:
			...
			strFontHeight = fontStruct->ascent + fontStruct->descent;

			XDrawString(display,window,gc,winWidth / 2 - strWidth / 2,winHeight / 2 - strFontHeight / 2 ,str,strLength);

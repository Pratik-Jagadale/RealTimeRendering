...
void drawShapes(void)
{
	...
	void drawSecondDiagram(void);
	void drawThirdDiagram(void);
	void drawFourthDiagram(void);
	void drawFifthDiagram(void);
	void drawSixthDiagram(void);
...
void drawPoints(void)
{
	...
	for (float j = 1.0f; j > -1.1f; j = j - 0.5f)
	{
		for (float i = -1.0f; i < 1.1f; i = i + 0.5f)
		{
			glVertex3f(i, j, 0.0f);
		}
	}
...
void drawFifthDiagram(void)
{
	...
	for (float i = -1.0f; i < 1.1f; i = i + 0.5f)
	{
		// upper downward lines
		glVertex3f(-1.0f, 1.0f, 0.0f);
		glVertex3f(1.0, -i, 0.0f);

		// lower downward lines
		glVertex3f(-1.0f, 1.0f, 0.0f);
		glVertex3f(i, -1.0f, 0.0f);
	}
...
{
	// color quad
	for (float i = -1.1f; i < 0.7f; i = i + 0.7f)
	{
		glBegin(GL_QUADS);
		if (i == -1.1f)
		...
		glVertex3f(i + 0.7f, 1.0f, 0.0f);
		glVertex3f(i, 1.0f, 0.0f);
		glVertex3f(i, -1.1f, 0.0f);
		glVertex3f(i + 0.7f, -1.1f, 0.0f);
		glEnd();
	}

	// Lines
	for (float i = -1.1f; i < 1.2f; i = i + 0.7f)
	{
		glBegin(GL_LINES);
		glColor3f(1.0f, 1.0f, 1.0f);
		// Horizontal
		glVertex3f(-1.1f, i, 0.0f);
		glVertex3f(1.0f, i, 0.0f);

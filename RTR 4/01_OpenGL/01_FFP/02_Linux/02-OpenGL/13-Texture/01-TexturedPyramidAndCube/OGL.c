...
int main(void)
{
	...
	if (ret == -1)
	{
		printf("loadGLTexture() texture failed for Kundali.bmp...exitting now\n");
		exit(1);
	}
	else if (ret == -2)
	{
		printf("loadGLTexture() texture failed for Stone.bmp...exitting now\n");
		exit(1);
	}
	// Game Loop
	while (bDone == False)
...
{
	// code
	AnglePyramid = AnglePyramid + 1.00f;

...
struct CBUFFER
{
	...
	XMVECTOR la[3], ld[3], ls[3], lightPosition[3], ka, kd, ks;
...
struct Light
{
	float lightAmbiant[4];
	float lightDiffuse[4];
	float lightSpecular[4];
	float lightPositions[4];
};
...
float materialShineeness = 50.0f;
BOOL bLight = FALSE;
...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"float3 viewerVector[3];"
		"float3 specular[3];"

		"for(int i = 0 ; i < 3 ; i++)"
		"{"
		"lightDirection[i] = normalize( (float3)lightPosition[i] - (float3)eyeCoordinate );"
		"ambiant[i] = la[i] * ks;"
		"diffuse[i] = ld[i] * kd * max(dot(lightDirection[i],transformedNormals), 0.0);"
		"reflectionVector[i] = reflect(-lightDirection[i],transformedNormals);"
		"viewerVector[i] = normalize(-eyeCoordinate.xyz);"
		"specular[i] = ls[i] * ks * pow(max(dot(reflectionVector[i],viewerVector[i]), 0.0),materialShininess);"
		"color = color + ambiant[i] + diffuse[i] + specular[i];"
		"}"
	...
	lights[0].lightPositions[0] = -2.0f;
	lights[0].lightPositions[1] = 0.0f;
	lights[0].lightPositions[2] = 0.0f;
	lights[0].lightPositions[3] = 1.0f;
	...
	lights[1].lightPositions[0] = 0.0f;
	lights[1].lightPositions[1] = 0.0f;
	lights[1].lightPositions[2] = 0.0f;
	lights[1].lightPositions[3] = 1.0f;
	...
	lights[2].lightPositions[0] = 0.0f;
	lights[2].lightPositions[1] = 0.0f;
	lights[2].lightPositions[2] = 0.0f;
	lights[2].lightPositions[3] = 1.0f;
...
void display(void)
{
	...
	constantBuffer.viewMatrix = viewdMatrix;
	...
	if (bLight == TRUE)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			constantBuffer.la[i] = XMVectorSet(lights[i].lightAmbiant[0], lights[i].lightAmbiant[1], lights[i].lightAmbiant[2], lights[i].lightAmbiant[3]);
			constantBuffer.ld[i] = XMVectorSet(lights[i].lightDiffuse[0], lights[i].lightDiffuse[1], lights[i].lightDiffuse[2], lights[i].lightDiffuse[3]);
			constantBuffer.ls[i] = XMVectorSet(lights[i].lightSpecular[0], lights[i].lightSpecular[1], lights[i].lightSpecular[2], lights[i].lightSpecular[3]);
			constantBuffer.lightPosition[i] = XMVectorSet(lights[i].lightPositions[0], lights[i].lightPositions[1], lights[i].lightPositions[2], lights[i].lightPositions[3]);
		}
		...
		constantBuffer.materialShininess = materialShineeness;

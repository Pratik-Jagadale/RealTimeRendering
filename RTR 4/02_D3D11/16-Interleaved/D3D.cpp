...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"float4 color:COLOR;"
		"float3 diffuse_light_color:LIGHT_COLOR;"
	...
	const char *pixelShaderSourceCode =
		...
		"{"
		"float4 color;"
		"color =  input.color * myTexture2D.Sample(mySamplerState, input.texcoord) * float4(input.diffuse_light_color,1.0);"
		"return color;"
		"}";
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "loadD3DTexture() Failed for marble in initialize()...\n");
		fclose(gpFile);
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "loadD3DTexture() Successful for marble in initialize()...\n");
		fclose(gpFile);
	}
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_PositionBuffer_Cube, &stride, &offset);

	// SET NORMAL BUFFER INTO IA STAGE IN PIPELINE
	stride = 11 * sizeof(float);
	offset = 6 * sizeof(float);

	gpID3D11DeviceContext->IASetVertexBuffers(2, 1, &gpID3D11Buffer_PositionBuffer_Cube, &stride, &offset);

	// SET TEXCOORD BUFFER INTO IA STAGE IN PIPELINE
	stride = 11 * sizeof(float);
	offset = 9 * sizeof(float);

	gpID3D11DeviceContext->IASetVertexBuffers(3, 1, &gpID3D11Buffer_PositionBuffer_Cube, &stride, &offset);
	...
	XMMATRIX r1 = XMMatrixRotationX(XMConvertToRadians(angleTriangle));
	XMMATRIX r2 = XMMatrixRotationY(XMConvertToRadians(angleTriangle));
	XMMATRIX r3 = XMMatrixRotationZ(XMConvertToRadians(angleTriangle));

...
void display(void)
{
	...
	XMMATRIX rotationMatrix_elbow = XMMatrixIdentity();
	...
	pushMatrix(viewdMatrix);
	{
		...
		// 	rotationMatrix = XMMatrixRotationY(XMConvertToRadians(shoulder));
		// 	translationMatrix = XMMatrixTranslation(1.0f, 0.0f, 0.0f);
		// 	rotationMatrix_elbow = XMMatrixRotationY(XMConvertToRadians(elbow));

		// 	scaleMatrix = XMMatrixScaling(0.2f, 0.2f, 0.2f);

		// 	worldMatrix = worldMatrix * scaleMatrix;
		// 	worldMatrix = worldMatrix * rotationMatrix_elbow;

...
ID3D11Buffer *gpID3D11Buffer_PositionBuffer_Triagle = NULL;
...
HRESULT initialize(void)
{
	...
	float CircleVertexdata[1080];
	int numberOfVerticesOfCircle = 0;

	numberOfVerticesOfCircle = getNumberOfCircleVertices();
	getCircleVertexData(CircleVertexdata);

	// CREATE VERTEX BUFFER FOR ABOVE TRIANGLE VERTEX POSITIONS
	...
	// C. CREATE THE BUFFER
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubResourceData, &gpID3D11Buffer_PositionBuffer_Triagle);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, "Log.txt", "a+");
	...
	// CREATE VERTEX BUFFER FOR ABOVE RECTANGLE VERTEX POSITIONS
	// A. INITIALIZE  BUFFER DESC
	ZeroMemory((void *)&d3d11BufferDescriptor, sizeof(D3D11_BUFFER_DESC));
	...
	// C. CREATE THE BUFFER
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubResourceData, &gpID3D11Buffer_PositionBuffer_Rectangle);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "gpID3D11Device::CreateBuffer FAILED for Position in initialize().\n");
	...
	d3d11BufferDescriptor.Usage = D3D11_USAGE_DEFAULT;
	d3d11BufferDescriptor.ByteWidth = sizeof(float) * _ARRAYSIZE(graphVertices);
	d3d11BufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;

	// B. INITIALIZE SUB RESOURCE DATA STRUCTURE TO PUT DATA INTO THE BUFFER
	ZeroMemory((void *)&d3d11SubResourceData, sizeof(D3D11_SUBRESOURCE_DATA));

	d3d11SubResourceData.pSysMem = graphVertices;
	...
	d3d11BufferDescriptor.Usage = D3D11_USAGE_DEFAULT;
	d3d11BufferDescriptor.ByteWidth = sizeof(float) * _ARRAYSIZE(CircleVertexdata);
	d3d11BufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;

	// B. INITIALIZE SUB RESOURCE DATA STRUCTURE TO PUT DATA INTO THE BUFFER
	ZeroMemory((void *)&d3d11SubResourceData, sizeof(D3D11_SUBRESOURCE_DATA));

	d3d11SubResourceData.pSysMem = CircleVertexdata;
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_PositionBuffer_Triagle, &stride, &offset);
	...
	ZeroMemory((void *)&constantBuffer, sizeof(CBUFFER));
	constantBuffer.wordViewProjectionMatrix = wvpMatrix;
	constantBuffer.color = XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f);

	// C. PUSH THEM INTO THE SHADER
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer, 0, NULL, &constantBuffer, 0, 0);

	// DRAW THE PRIMITIVE
	gpID3D11DeviceContext->Draw(100, 0);
	...
	// DRAW THE PRIMITIVE
	gpID3D11DeviceContext->Draw(1028, 0);
...
void uninitialize(void)
{
	...
	if (gpID3D11Buffer_PositionBuffer_Triagle)
	{
		gpID3D11Buffer_PositionBuffer_Triagle->Release();
		gpID3D11Buffer_PositionBuffer_Triagle = NULL;
	}

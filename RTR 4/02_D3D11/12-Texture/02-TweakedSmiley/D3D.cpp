...
void display(void)
{
	...
	// CODE
	// Set TexCoords Depending on KeyPressed
	float textCoord[12];
	if (keyPressed == 1)
	{
		textCoord[0] = +0.0f;
	...
	d3d11BufferDescriptor.Usage = D3D11_USAGE_DEFAULT;
	d3d11BufferDescriptor.ByteWidth = sizeof(float) * _ARRAYSIZE(textCoord);
	d3d11BufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	...
	d3d11SubResourceData.pSysMem = textCoord;

	// C. CREATE THE BUFFER
	HRESULT hr = gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubResourceData, &gpID3D11Buffer_TextureBuffer_Square);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "gpID3D11Device::CreateBuffer FAILED for TexCoord in initialize().\n");
	...
	if (keyPressed == 1)
	{
		constantBuffer.keyPressUniform = 1;
	}
	else if (keyPressed == 2)
	{
		constantBuffer.keyPressUniform = 1;
	...
	else if (keyPressed == 4)
	{
		...
		constantBuffer.keyPressUniform = 0;

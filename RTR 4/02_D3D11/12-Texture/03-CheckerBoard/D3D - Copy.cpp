...
HRESULT loadD3DTexture(ID3D11ShaderResourceView **ppID3D11ShaderResourceView)
{
	...
	// hr = DirectX::CreateWICTextureFromMemory(gpID3D11Device, gpID3D11DeviceContext, binaryData, sizeof(checkerboard), &texture, ppID3D11ShaderResourceView);
...
void MakeCheckerBoard(void)
{
	// Variable Declartions
	int c;
	checkerboard = (uint8_t *)malloc(sizeof(uint8_t) * CHECKERBOARD_WIDTH * CHECKERBOARD_HEIGHT * 4);
	...
	for (int i = 0; i < CHECKERBOARD_WIDTH; i++)
	{
		for (int j = 0; j < CHECKERBOARD_HEIGHT; j++)
		{
			...
			fprintf(gpFile, "%d\n", c);
			checkerboard[(i * 64 + j) * 4 + 0] = (uint8_t)c;
			checkerboard[(i * 64 + j) * 4 + 0] = (uint8_t)c;
			checkerboard[(i * 64 + j) * 4 + 0] = (uint8_t)c;
			checkerboard[(i * 64 + j) * 4 + 0] = (uint8_t)255;
		}
	}
}

...
HRESULT initialize(void)
{
	...
	// CREATE TEXTURE
	hr = loadD3D_DDSTexture(MAKEINTRESOURCE(SMILEY_DDS_TEXTURE), gpID3D11Device, &gpID3D11ShaderResourceView_Texture);
...
HRESULT loadD3D_DDSTexture(TCHAR ddsTextureResourceID[], ID3D11Device *pID3D11Device, ID3D11ShaderResourceView **ppID3D11ShaderResourceView)
{
	...
	void *pTextureResourceImageData = nullptr;
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "loadD3D_DDSTexture : FindResource() Failed  ...\n");
		fclose(gpFile);
		return (E_FAIL);
	}
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "loadD3D_DDSTexture : LoadResource() Failed  ...\n");
		fclose(gpFile);
		return (E_FAIL);
	}

	// Get Starting byte address of resource loaded in memory
	pTextureResourceImageData = LockResource(hGlobal);
	if (pTextureResourceImageData == nullptr)
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "loadD3D_DDSTexture : LockResource() Failed  ...\n");
		fclose(gpFile);
		return (E_FAIL);
	}

	dwTextureResourceSize = SizeofResource(GetModuleHandle(NULL), hResource);
	if (dwTextureResourceSize == 0)
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "loadD3D_DDSTexture : SizeofResource() Failed  ...\n");
		fclose(gpFile);
		return (E_FAIL);
	}

	hr = DirectX::CreateDDSTextureFromMemory(pID3D11Device,
											 (const uint8_t *)pTextureResourceImageData,
											 (size_t)dwTextureResourceSize,
											 NULL,
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "loadD3D_DDSTexture : DirectX::CreateDDSTextureFromMemory() Failed  ...\n");
		fclose(gpFile);
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "loadD3D_DDSTexture : DirectX::CreateDDSTextureFromMemory() Successful  ...\n");
		fclose(gpFile);
	}
	return hr;
}

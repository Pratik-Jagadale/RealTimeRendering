...
ID3D11ShaderResourceView *gpID3D11ShaderResourceView_TextureKundali = NULL;
ID3D11ShaderResourceView *gpID3D11ShaderResourceView_TextureStone = NULL;
...
HRESULT initialize(void)
{
	...
	// CREATE VERTEX BUFFER FOR ABOVE VERTEX POSITIONS - Pyramid
	...
	d3d11BufferDescriptor.Usage = D3D11_USAGE_DEFAULT;
	d3d11BufferDescriptor.ByteWidth = sizeof(float) * _ARRAYSIZE(PyramidTexCoord);
	d3d11BufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	...
	d3d11BufferDescriptor.Usage = D3D11_USAGE_DEFAULT;
	d3d11BufferDescriptor.ByteWidth = sizeof(float) * _ARRAYSIZE(CubeTexCoord);
	d3d11BufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	...
	// CREATE TEXTURE
	hr = loadD3DTexture(L"Stone.bmp", &gpID3D11ShaderResourceView_TextureStone);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "loadD3DTexture() Failed for Stone in initialize()...\n");
		fclose(gpFile);
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "loadD3DTexture() Successful for Stone in initialize()...\n");
		fclose(gpFile);
	}

	hr = loadD3DTexture(L"Vijay_Kundali.bmp", &gpID3D11ShaderResourceView_TextureKundali);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, "Log.txt", "a+");
...
void display(void)
{
	...
	gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_TextureStone);
	...
	viewdMatrix = XMMatrixIdentity();
	translationMatrix = XMMatrixIdentity();
	rotationMatrix = XMMatrixIdentity();
	XMMATRIX r1 = XMMatrixRotationX(XMConvertToRadians(angle));
	XMMATRIX r2 = XMMatrixRotationY(XMConvertToRadians(angle));
	XMMATRIX r3 = XMMatrixRotationZ(XMConvertToRadians(angle));
	...
	wvpMatrix = worldMatrix * viewdMatrix * perspectiveProjectionMatrix;
	...
	// SET SHADER VIEW INTO PIXEL SHADER
	gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_TextureKundali);
...
void uninitialize(void)
{
	...
	if (gpID3D11ShaderResourceView_TextureKundali)
	{
		gpID3D11ShaderResourceView_TextureKundali->Release();
		gpID3D11ShaderResourceView_TextureKundali = NULL;
	}

	if (gpID3D11ShaderResourceView_TextureStone)
	{
		gpID3D11ShaderResourceView_TextureStone->Release();
		gpID3D11ShaderResourceView_TextureStone = NULL;
	}

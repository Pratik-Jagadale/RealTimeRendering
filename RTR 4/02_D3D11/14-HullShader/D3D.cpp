...
ID3D11DomainShader *gpID3D11DomainShader = NULL; // Tesselation Evaluations
...
unsigned int numberOfLineSegment;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_UP:
			numberOfLineSegment++;
			if (numberOfLineSegment >= 30)
			{
				numberOfLineSegment = 30;
			}
			break;
...
HRESULT initialize(void)
{
	...
	const char *hullShaderSourceCode =
		...
		"{"
		"float edeges[2]:SV_TESSFACTOR;"
		"};"
		"hull_constant_output hullConstatFunction(void)"
		"{"
		"hull_constant_output output;"
		"float numberOfStrips = hull_constant_function_param[0];"
		"float numberOfSegment = hull_constant_function_param[1];"
		"output.edeges[0] = numberOfStrips;"
		"output.edeges[1] = numberOfSegment;"
		"return output;"
		"}"
		"struct hull_output"
		"{"
		...
		"[patchconstantfunc(\"hullConstatFunction\")]"
	...
	if (FAILED(hr))
	{
		...
		if (pID3DBlob_error)
		{
			fprintf(gpFile, "D3DCompile() Failed for Hull Shader :\n%s.\n", (char *)pID3DBlob_error->GetBufferPointer());
		}
		else
		{
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "ID3D11Device::CreateHullShader FAILED in initialize().\n");
		fclose(gpFile);
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "ID3D11Device::CreateHullShader SUCCEEDED in initialize().\n");
		fclose(gpFile);
	}

	// SET THIS HULL SHADER IN HULL SHADER STAGE OF PIPE LINE
	...
	const char *domainShaderSourceCode =
		"cbuffer constantBuffer"
		"{"
		"float4x4 wordViewProjectionMatrix;"
		"}"
		"struct hull_constant_output"
		"{"
		"float edeges[2]:SV_TESSFACTOR;"
		...
		"domain_output main(hull_constant_output input, OutputPatch <hull_output, 4> outputpatch, float2 texCoord:SV_DOMAINLOCATION)"
		"{"
		...
		"float3 p = p0 * (1 - texCoord.x) * (1 - texCoord.x) * (1 - texCoord.x) + "
		" 			p1 * 3 * texCoord.x * (1 - texCoord.x) * (1 - texCoord.x) +"
		"			p2 * 3 * texCoord.x * texCoord.x * (1 - texCoord.x) +"
		"			p3 * 3 * texCoord.x * texCoord.x * texCoord.x;"

		"output.position = mul(wordViewProjectionMatrix , float4(p , 1.0));"
		"return output;"
		"}";
	...
	if (FAILED(hr))
	{
		...
		if (pID3DBlob_error)
		{
			fprintf(gpFile, "D3DCompile() Failed for Domain Shader :\n%s.\n", (char *)pID3DBlob_error->GetBufferPointer());
		}
		else
		{
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "ID3D11Device::CreateDomainShader FAILED in initialize().\n");
		fclose(gpFile);
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "ID3D11Device::CreateDomainShader SUCCEEDED in initialize().\n");
		fclose(gpFile);
	}

	// SET THIS DOMAIN SHADER IN HULL SHADER STAGE OF PIPE LINE
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "gpID3D11Device::CreateBuffer FAILED for ConstantBuffer_HullShader in initialize().\n");
		fclose(gpFile);
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "gpID3D11Device::CreateBuffer SUCCEEDED for ConstantBuffer_HullShader in initialize().\n");
		fclose(gpFile);
	}

	// C. SET CONSTANT BUFFER IN HULL SHADER STAGE OF PIPELINE
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "gpID3D11Device::CreateBuffer FAILED for ConstantBuffer_DomainShader in initialize().\n");
		fclose(gpFile);
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "gpID3D11Device::CreateBuffer SUCCEEDED for ConstantBuffer_DomainShader in initialize().\n");
		fclose(gpFile);
	}

	// C. SET CONSTANT BUFFER IN DOMAIN SHADER STAGE OF PIPELINE
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "gpID3D11Device::CreateBuffer FAILED for ConstantBuffer_PixelShader in initialize().\n");
		fclose(gpFile);
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "gpID3D11Device::CreateBuffer SUCCEEDED for ConstantBuffer_PixelShader in initialize().\n");
		fclose(gpFile);
	}

	// C. SET CONSTANT BUFFER IN PIXEL SHADER STAGE OF PIPELINE
	gpID3D11DeviceContext->PSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer_PixelShader);

	// ENABALING RASTERIZER STATE
...
void display(void)
{
	...
	constantBuffer_DomainShader.wordViewProjectionMatrix = wvpMatrix;
	...
	// UPDATE HULL SHADER CONSTANT BUFFER
	// B. PUT THEM INTO CONSTANT BUFFERS
	CBUFFER_HULL_SHADER constantBuffer_HullShader;
	ZeroMemory((void *)&constantBuffer_HullShader, sizeof(CBUFFER_HULL_SHADER));
	constantBuffer_HullShader.Hull_Constant_Function_Params = XMVectorSet(1.0f, (FLOAT)numberOfLineSegment, 0.0f, 1.0f);

	// C. PUSH THEM INTO THE SHADER
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_HullShader, 0, NULL, &constantBuffer_HullShader, 0, 0);

	TCHAR str[255];
	wsprintf(str, TEXT("D3D11 - Pratik Rajendra Jagadale | Number of Segments : %d"), numberOfLineSegment);

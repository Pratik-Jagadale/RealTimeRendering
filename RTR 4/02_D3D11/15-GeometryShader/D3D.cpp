...
HRESULT initialize(void)
{
	...
	// GEOMETRY SHADER
	const char *geomtryShaderSourceCode =
		"cbuffer ConstantBuffer"
		"{"
		"float4x4 worldViewProjectionMatrix;"
		"}"
	...
	// COMPILE VERTEX SHADER
	hr = D3DCompile(geomtryShaderSourceCode,
					lstrlenA(geomtryShaderSourceCode) + 1,
					"GS",
					NULL,
					D3D_COMPILE_STANDARD_FILE_INCLUDE,
					"main",
					"gs_5_0", // FEATURE LEVEL
					0,
					0,
	...
	if (FAILED(hr))
	{
		...
		if (pID3DBlob_error)
		{
			fprintf(gpFile, "D3DCompile() Failed for Geometry Shader :\n%s.\n", (char *)pID3DBlob_error->GetBufferPointer());
		}
		else
		{
			fprintf(gpFile, "D3DCompile() Failed for Goemetry Shader : Unknown.\n");
		}
		fclose(gpFile);
		return (hr);
	}
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "ID3D11Device::CreateGeometryShader FAILED in initialize().\n");
		fclose(gpFile);
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "ID3D11Device::CreateGeometryShader SUCCEEDED in initialize().\n");
		fclose(gpFile);
	}

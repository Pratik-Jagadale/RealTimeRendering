...
struct MyPoint point_PRJ; // declaring a single variable of type 'struct MyPoint' globally

struct MyPointProperties point_properties; //delcaring a single variable of type 'struct MyPointProperties' globaly

int main(void)
{
    //code
    //User Input for The data Member Of struct MyPoint Variables point_PRJ;
    printf("\n\n");
    ...
    if (point_PRJ.x == 0 && point_PRJ.y == 0)
        printf("The Point Is On The Orgin (%d , %d) !!!\n\n", point_PRJ.x, point_PRJ.y);
    else
    {
        if (point_PRJ.x == 0)
        {

...
//Global Variables
float gfTranslate = 3.0f;
float gyTranslate = 3.0f;
float gzTranslate = 3.0f;
#define PI 3.1459265
...
void display(void)
{
	...
	glLoadIdentity();
	glTranslatef(0.0f, gfTranslate, -9.0f);
	...
	// Circle
	glLoadIdentity();
	glTranslatef( 0.0f, 0.0f, gfTranslate - 9.0f);

	for(float angle = 0 ; angle < PI * 2 ; angle = angle  + 0.001 )
	{
		glBegin(GL_LINES);

...
float xTrangle = 0.0f, yTrangle = 0.0f;
float xRectangle = 0.0f, yRectangle = 0.0f;
...
void display(void)
{
	...
	glTranslatef( xTrangle, yTrangle, -9.0f); 
	...
	glTranslatef( xRectangle, yRectangle, -9.0f); 
    glRotatef(AngleRectangle,1.0f,0.0f,0.0f); // Spinning
...
void update(void)
{
	...
	AngleRectangle = AngleRectangle + 0.3f;
    if(AngleRectangle >= 360.0f)
        AngleRectangle = -360.0f;

	AngleTrangle = AngleTrangle + 0.03f;
	if(AngleTrangle >= 360.0f)
		AngleTrangle = -360.0f;

	if(angle < PI * 2)
	{
		xTrangle = 2.5f * cos(angle);
		yTrangle = 2.5f * sin(angle);

		xRectangle = 2.5 * cos(-angle);
		yRectangle = 2.5 * sin(-angle);
